...
class ShortcutParser(object):
  ...
  def match_base(self,string):
    match = re.match(u(r"\u0000shortcuts\u0000(.*)\u0008\u0008$"), string, re.IGNORECASE)
    if match:
  ...
  def match_array_string(self,string):
    ...
    shortcuts = []
    while True:
      match = re.match(u(r"(.*)\u0000[0-9]+\u0000(\u0001AppName.*)\u0008") ,string, re.IGNORECASE)
      if match:
        groups = match.groups()
        string = groups[0]
  ...
  def match_shortcut_string(self,string):
    ...
    match = re.match(u(r"\u0001AppName\u0000(.*)\u0000\u0001Exe\u0000(.*)\u0000\u0001StartDir\u0000(.*)\u0000\u0001icon\u0000(.*)\u0000\u0000tags\u0000(.*)\u0008"), string, re.IGNORECASE)
    if match:
